...
object SparkALS {
	...
	// generate rating data from two factorized matrices, movies_factor matrix  and users_factor matrix
	...
	// obtain rmse between the real rating and estimated rating generated by ALS
  	def rmse(targetR: DoubleMatrix2D, ms: Array[DoubleMatrix1D],us: Array[DoubleMatrix1D]): Double = {
	...
	// 
  	def update(i: Int, m: DoubleMatrix1D, us: Array[DoubleMatrix1D],R: DoubleMatrix2D) : DoubleMatrix1D = {
		val U = us.size
		val F = us(0).size
  	...
  	def main(args: Array[String]) {
    		...
    		val sc = new SparkContext(host, "SparkALS", System.getenv("SPARK_HOME"), SparkContext.jarOfClass(this.getClass))
		...
		// broadcast syntax: http://spark.apache.org/docs/0.9.1/scala-programming-guide.html#broadcast-variables
    		val Rc  = sc.broadcast(R)
    		var msb = sc.broadcast(ms)
    		var usb = sc.broadcast(us)
    		for (iter <- 1 to ITERATIONS) {
      			println("Iteration " + iter + ":")
      			ms = sc.parallelize(0 until M, slices).map(i => update(i, msb.value(i), usb.value, Rc.value)).toArray
